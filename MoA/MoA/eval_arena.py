"""
Usage:
python gen_moa_judgment.py --model-list [LIST-OF-MODEL-ID] --parallel [num-concurrent-api-call] --mode [single|pairwise-baseline|pairwise-all]
"""
import json
import yaml
import argparse
...
if __name__ == "__main__":
    ...
    parser.add_argument("--setting-file", type=str, default="arena_hard_auto/config/judge_config.yaml")
    ...
    question_file = os.path.join("arena_hard_auto/data", configs["bench_name"], "question.jsonl")
    answer_dir = os.path.join("arena_hard_auto/data", configs["bench_name"], "model_answer")
    ref_answer_dir = os.path.join("arena_hard_auto/data", configs["bench_name"], "reference_answer")
    ...
    output_files = {}
    output_dir = f"arena_hard_auto/data/{configs['bench_name']}/model_judgment/{configs['judge_model']}"
    for model in models:
